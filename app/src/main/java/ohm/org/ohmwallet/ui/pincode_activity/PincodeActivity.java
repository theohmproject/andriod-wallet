package ohm.org.ohmwallet.ui.pincode_activity;
...
import ohm.org.ohmwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import ohm.org.ohmwallet.ui.base.BaseActivity;
import ohm.org.ohmwallet.ui.settings_pincode_activity.KeyboardFragment;
import ohm.org.ohmwallet.ui.start_activity.StartActivity;

import static ohm.org.ohmwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (ohmApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (ohmApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            ohmApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
            ohmApplication.stopBlockchain();
        }

        ohmApplication.getAppConf().setAppInit(true);
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        ohmApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(ohmApplication.getAppConf().getPincode().equals(pincode)){
