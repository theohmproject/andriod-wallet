...
import ohm.org.ohmwallet.ui.contacts_activity.ContactsActivity;
import ohm.org.ohmwallet.ui.donate.DonateActivity;
import ohm.org.ohmwallet.ui.settings_activity.SettingsActivity;
import ohm.org.ohmwallet.ui.wallet_activity.WalletActivity;

import static ohm.org.ohmwallet.module.Coin2PlayContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends Coin2PlayActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = ohmApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = ohmModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (ohmModule.getChainHeight()+10>peerHeight) {
            ...
            if (ohmModule.isAnyPeerConnected()) {
                long peerHeight = ohmModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (ohmModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = ohmModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (ohmModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
